...
export class MediaDoc extends MainDoc {
  ...
  protected temp() {
    ...
    const similarStanza: HTMLElement = document.createElement(
      "togostanza-gmdb_meta_list"
    );
    similarStanza.setAttribute("limit", "10");
    similarStanza.setAttribute("column_names", "true");
    similarStanza.setAttribute(
      "api_url",
      `http://growthmedium.org/sparqlist/api/gmdb_list_similar_media_by_gmid?gm_id=${param}`
    );
    ...
    const organismsStanza: HTMLElement = document.createElement(
      "togostanza-gmdb_meta_list"
    );
    organismsStanza.setAttribute("limit", "10");
    organismsStanza.setAttribute("column_names", "true");
    organismsStanza.setAttribute(
      "api_url",
      `http://growthmedium.org/sparqlist/api/gmdb_organisms_by_gmid?gm_id=${param}`
    );
